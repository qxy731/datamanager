...
//			pojo.setKey1("SALESCODE");
	...
	public static Map<String,String> getDataQuerySQL(String[] fileNames) {
		...
		sqlMap.put("20", "SELECT APPLICATIONDATE,COUNT(1) AS COUNT FROM POCTEST3_DEST GROUP BY APPLICATIONDATE");
		sqlMap.put("21", "SELECT APPLICATIONDATE,COUNT(1) AS COUNT FROM POCTEST3_DEST WHERE APPROVALRESULT='通过' GROUP BY APPLICATIONDATE");
		sqlMap.put("22", "SELECT APPLICATIONDATE,COUNT(1) AS COUNT FROM POCTEST3_DEST WHERE ALARMCODE IN ('H','S') GROUP BY APPLICATIONDATE ");
		sqlMap.put("23", "SELECT APPLICATIONDATE,COUNT(1) AS COUNT FROM POCTEST3_DEST WHERE RESEARCHCONCLUSION='K' GROUP BY APPLICATIONDATE ");
		sqlMap.put("24", "SELECT APPLICATIONDATE,COUNT(1) AS COUNT FROM POCTEST3_DEST WHERE RESEARCHCONCLUSION IN('K','F') GROUP BY APPLICATIONDATE ");
